...
public class MailSchedulerService {
	...
	protected void handleMailSending(MailLog mailLog) {
		...
		try {
			...
			changedMailLog = new MailLog(mailLog.getId(), mailLog.getUserId(), mailLog.getEmail(), MailStatus.SUCCESS,
				mailLog.getContent(), mailLog.getCreateTime(), Instant.now(), mailLog.getCountRT());

		} catch (Exception e) {
			MailStatus status = MailStatus.FAIL;
			if (mailLog.getCountRT() >= 3)
				status = MailStatus.PERMANENT_FAIL;

			changedMailLog = new MailLog(mailLog.getId(), mailLog.getUserId(), mailLog.getEmail(), status,
				mailLog.getContent(), mailLog.getCreateTime(), Instant.now(), mailLog.getCountRT() + 1);
